...
export async function POST(req: NextRequest) {
    try {
        ...
        const seedMarkerRef = adminDb.collection('_metadata').doc('seed_status');
        const seedMarker = await seedMarkerRef.get();
        if (seedMarker.exists && seedMarker.data()?.seeded === true) {
            return NextResponse.json(
                { error: 'Database has already been seeded. Delete _metadata/seed_status to re-seed.', code: 'already-exists' },
                { status: 409 },
            );
        }
        ...
        for (const trainer of TRAINERS) {
            const ref = adminDb.collection('trainers').doc(trainer.id);
            batch.set(ref, { ...trainer, createdAt: now, updatedAt: now });
        }

        // 2. Seed Subscription Plans
        for (const plan of SUBSCRIPTION_PLANS) {
            const ref = adminDb.collection('subscriptionPlans').doc(plan.id);
            batch.set(ref, { ...plan, createdAt: now, updatedAt: now });
        }

        // 3. Seed Gym Center
        const gymRef = adminDb.collection('gymCenters').doc(GYM_CENTER.id);
        batch.set(gymRef, { ...GYM_CENTER, createdAt: now, updatedAt: now });

        // Commit the first batch
        await batch.commit();

        // 4. Seed Classes (next 7 days)
        ...
        return NextResponse.json(
            { error: 'Failed to seed database', code: 'internal' },
            { status: 500 },
        );
    }
}
