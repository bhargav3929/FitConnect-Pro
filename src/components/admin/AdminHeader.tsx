...
export function AdminHeader() {
    ...
    return (
        <header className="h-20 border-b border-peach-400/15 bg-peach-50/80 backdrop-blur-xl sticky top-0 z-30 transition-all duration-300">
            <div className="h-full px-4 lg:px-8 flex items-center justify-between">
                ...
                <div className="flex items-center gap-4">
                    ...
                    <button
                        ...
                        className="lg:hidden w-10 h-10 flex items-center justify-center text-olive-400 hover:text-olive-600 transition-colors rounded-lg active:bg-peach-300/30"
                    >
                    ...
                    <div className="flex flex-col">
                        ...
                        <p className="text-olive-300/60 text-[10px] tracking-[0.2em] uppercase hidden sm:block">
                            SOL Administration
                        </p>
                    </div>
                </div>
                ...
                <div className="flex items-center gap-2 sm:gap-4">
                    ...
                    <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                            <button className="flex items-center gap-3 pl-2 pr-2 py-1.5 rounded-full hover:bg-peach-300/30 border border-transparent hover:border-peach-400/15 transition-all">
                                <Avatar className="h-8 w-8 ring-2 ring-peach-400/15">
                                    <AvatarFallback className="bg-gradient-to-br from-terra-400 to-terra-500 text-peach-50 font-bold text-xs">
                                        {adminUser?.name?.charAt(0) || 'A'}
                                    </AvatarFallback>
                                </Avatar>
                                <div className="hidden md:flex flex-col items-start">
                                    <p className="text-sm font-semibold text-olive-600 leading-none">
                                        {adminUser?.name || 'Admin'}
                                    </p>
                                    <p className="text-[10px] text-olive-300 leading-none mt-1">
                                        Super Admin
                                    </p>
                                </div>
                                <ChevronDown className="w-3 h-3 text-olive-300 hidden md:block" />
                            </button>
                        </DropdownMenuTrigger>
                        <DropdownMenuContent
                            align="end"
                            className="w-64 bg-peach-50/95 backdrop-blur-xl border-peach-400/15 shadow-2xl shadow-black/10 text-olive-600 p-1"
                        >
                            <div className="px-2 py-3 bg-peach-200/40 rounded-md mb-1 flex items-center gap-3">
                                ...
                                <div className="flex flex-col">
                                    <span className="font-semibold text-sm text-olive-600">{adminUser?.name || 'Admin User'}</span>
                                    <span className="text-xs text-olive-300">{adminUser?.email || 'admin@example.com'}</span>
                                </div>
                            </div>
